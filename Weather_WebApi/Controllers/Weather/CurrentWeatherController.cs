 ...
 */
    ...
    public class CurrentWeatherController : Controller
    {
        ...
        public CurrentWeatherController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpGet]
        public async Task<PartialViewResult> GetCurrentWeatherInfo(string zip, string country, string units, double? lat, double? lon)
   {
            string URL;
            ...
            if ((lat == null) || (lon == null))
            {
                ...
                if (units == "I")
                    URL = $"https://api.weatherbit.io/v2.0/current?&postal_code={zip}&country={country}&key={API_KEY}&units={units}";
            ...
            {
                if (units == "I")
                    URL = $"https://api.weatherbit.io/v2.0/current?&lat={lat}&lon={lon}&key={API_KEY}&units={units}";
                else //DEFAULT is Metric
            ...
            return PartialView("Weather/CurrentWeatherInfo_Partial",weatherForTheDay);
        }
